...
const PHONE = "+49 3831 4458726";
const PHONE_HREF = "tel:+4938314458726";
...
const inputClass = (hasError: boolean) =>
  cn(
    "w-full px-4 py-3.5 bg-cream border rounded-sm text-charcoal text-sm",
...
export default function ReservationForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      // Demo: ignore errors, show success
    }
    setSubmitted(true);
  };
  ...
  if (submitted) {
    return (
      <div className="py-16 text-center">
        ...
        <p className="mt-4 text-sm text-charcoal-muted">Wir freuen uns auf Ihren Besuch.</p>
      </div>
    );
  }
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
        ...
        <div className="flex flex-wrap gap-2">
          {PERSON_OPTIONS.map((n) => (
            <button
              ...
              className={cn(
                "w-12 h-12 text-sm font-medium rounded-sm border transition-all duration-150",
                watch("personen") === n
          ...
          <a
            href={PHONE_HREF}
            className="px-4 h-12 text-xs border border-dashed border-cream-border text-charcoal-muted rounded-sm flex items-center"
      ...
      <div>
        ...
        <textarea
          ...
          {...register("nachricht")}
          className={cn(
            "w-full px-4 py-3.5 bg-cream border border-cream-border rounded-sm text-charcoal text-sm",
            "placeholder:text-charcoal-muted focus:outline-none focus:border-charcoal-muted transition-colors duration-150",
      ...
      <div className="flex items-start gap-3">
        <input
          ...
          className="mt-0.5 w-4 h-4 accent-terracotta cursor-pointer flex-shrink-0"
        />
      ...
      {errors.datenschutz && (
        <p className="-mt-3 text-xs text-terracotta">{errors.datenschutz.message}</p>
      )}
      ...
      <button
        ...
        className={cn(
          "w-full h-12 label-sm rounded-sm transition-all duration-200",
